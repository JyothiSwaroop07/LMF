...
func main() {
	...
	// Register ECID engine server
	// pb.RegisterEcidEngineServiceServer(srv, server.NewEcidServer(logger))
	reflection.Register(srv)

	logger.Info("ECID Engine gRPC server starting", zap.String("addr", cfg.GRPC.ListenAddr()))
	...
	logger.Info("shutting down ECID Engine")
	srv.GracefulStop()
